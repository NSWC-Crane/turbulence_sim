...
% Read in images, use non-Laplacian option, call SSIM function.
% Steps:
% 1.  Read in reference and test image
% 2.  Subtract mean
% 3.  Get FFT
% 4.  Run SSIM metric
% 5.  Must adjust dynamicRange so that C1 and C2 don't cause the metric to
% approach 1.0 in all cases.
...
for index = 1:length(realFiles)
    ...
    patt1 = "r" + rng + "_z" + zm;
    patt2 = "_N1.png";  % Only selecting one simulated image per Cn2 value
    for i = 1:size(SimImgNames,2)
    ...
    % PRE-PROCESS REFERENCE (REAL) IMAGE
    % Read in reference(real) image - green channel only
    ImageR = double(imread(realFiles(index))); 
    ImageR= ImageR(:,:,2); 
    % Subtract mean
    if subtractMean == true
        ImageR = ImageR - mean(ImageR, 'all'); 
    end
    ...
    for  k = 1:length(simNamelist)
        ...
        % Remove border pixels
        ImageSim = ImageSim(6:end-5, 6:end-5);
