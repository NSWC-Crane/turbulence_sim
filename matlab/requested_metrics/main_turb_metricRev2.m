...
normalz = false; % And normalize data (subtract mean and divide by std)
meanonly = false;
rescaleI = false;
rescaleMoveEnds = true;
...
dirOut = data_root + "modifiedBaselines\NewSimulations\ByVaryingCn2\Plots\RescaleMoveEnds\";
...
for rng = rangeV
    for zm = zoom
        ...
        for i = 1:length(ImgNames1) 
            ...
            % Preprocess image
            if normalz == true
                vImgR{i,1} = (vImgR{i,1} - mean(vImgR{i,1},'all'))/std(vImgR{i,1},1,'all');
            end
            if meanonly == true
                vImgR{i,1}= vImgR{i,1} - mean(vImgR{i,1},'all');
            end
            if rescaleI == true
                vImgR{i,1}= rescale(vImgR{i,1});
            end
            if rescaleMoveEnds == true
                vImgR{i,1}= rescale(vImgR{i,1}) - 0.5;
            end
        end
...
for rngP = rangeV
    ...
    if savePlots == true
        f = gcf;
        fileN = fullfile(dirOut,"ALog_r" + num2str(rngP) + strPtch + ".png");
...
for rngP = rangeV
    ...
    for k = 1:length(zoom) %zmP = zoom
        ...
        h2.Color = plotcolors(k);
        str = "Z" + num2str(zoom(k)) + ": Max metric " + num2str(MMetric(ind1,1)) + " at cn2 " + num2str(MMetric(ind1,2));
        annotation('textbox',[.68 .5 .3 upY], ...
            'String',str,'EdgeColor','none')
        upY = upY-0.05;
    end
    ...
    if savePlots == true
        f = gcf;
        fileN = fullfile(dirOut,"ALogCn2_r" + num2str(rngP) + strPtch + ".png");
        %fileNf = fullfile(dirOut,"Logr" + num2str(rngP) + strPtch + ".fig");
        exportgraphics(f,fileN,'Resolution',300)
        %savefig(ffg,fileNf)
        %close(ffg)
